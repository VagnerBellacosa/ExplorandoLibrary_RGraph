...
{var args=RGraph.getArgs(arguments,'key,worksheet,callback');if(arguments.length===2){args.callback=arguments[1];args.worksheet=null;}
var worksheet,callback,letters='ABCDEFGHIJKLMNOPQRSTUVWXYZ',key=args.key;if(arguments.length===3){worksheet=Number(args.worksheet);callback=args.callback;}else{worksheet=1;callback=args.worksheet;}
var rand=RGraph.random(0,999999999);var url=('https://spreadsheets.google.com/feeds/cells/[KEY]/[WORKSHEET]/public/full?alt=json-in-script&callback=__rgraph_JSONPCallback'+rand).replace(/\[KEY\]/,key).replace(/\[WORKSHEET\]/,worksheet);this.load=function(url,userCallback)
{var obj=this;window['__rgraph_JSONPCallback'+rand]=function(json)
